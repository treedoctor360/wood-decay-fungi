...
// GETで全件取得（トークン＋共有キーワードをクエリで送る）
// group を渡すと、GAS側でそのキーワードに一致する記録だけが返る（混入防止）。
async function gasGetAll(group) {
...
// ============================================================
// 撮影ガイド: 撮っておきたい写真の視点(部位)と、なぜ重要か
//   同定の決め手は「裏面(子実層托)」「断面(組織色)」なので、それが写るよう促す。
// ============================================================
const PHOTO_VIEWS = [
  { key: "全体",           why: "発生位置・宿主との関係・群生の様子" },
  { key: "傘表面",         why: "色・環紋・毛・質感" },
  { key: "裏面(子実層托)", why: "ヒダ/管孔/針/平坦の別。最重要の決め手" },
  { key: "断面",           why: "組織(肉)の色・層・変色(D1タバコウロコタケ科の判別)" },
];
const PHOTO_VIEW_OPTIONS = ["未指定", "全体", "傘表面", "裏面(子実層托)", "断面", "その他"];
...
export default function App() {
  ...
  const [userNotes, setUserNotes] = useState([]); // 対話でユーザーが追加した自由記述(AIに渡す)
  ...
  const [showChat, setShowChat] = useState(true); // 同定チャットの表示/非表示
  ...
  const [groupKeyword, setGroupKeyword] = useState(""); // 共有キーワード(グループ合言葉)。同じ値の人だけで記録を共有する
  ...
  async function doSync() {
    ...
    try {
      ...
      // 1. 未同期レコードをアップ(共有キーワードgroupを付与)
      const unsynced = all.filter(r => !r.synced && !r.deleted);
      if (unsynced.length > 0) {
        await gasPost({ action: "save", records: unsynced.map(r => toGasRecord({ ...r, group: r.group || groupKeyword })) });
        for (const r of unsynced) await idbPut({ ...r, group: r.group || groupKeyword, synced: true });
      }

      // 2. 削除待ち(tombstone=削除印)をGASに送って端末からも消す(groupも送り、GAS側で同一グループのみ削除)
      const toDelete = all.filter(r => r.deleted);
      if (toDelete.length > 0) {
        await gasPost({ action: "delete", ids: toDelete.map(r => r.record_id), group: groupKeyword });
        for (const r of toDelete) await idbRemove(r.record_id);
      }

      // 3. GASから同じ共有キーワードの記録だけダウンして端末に取り込む
      const gasRes = await gasGetAll(groupKeyword);
  ...
  // 起動時とオンライン復帰時に自動同期
  useEffect(() => {
    // 登録者名・共有キーワードをlocalStorageから復元
    const saved = localStorage.getItem("fungi_contributor") || "";
    setMyContributor(saved);
    const savedGroup = localStorage.getItem("fungi_group") || "";
    setGroupKeyword(savedGroup);
    ...
    files.forEach((file) => {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          setPhotos((prev) => [...prev, { url, base64, mime: "image/jpeg", view: "未指定" }]);
        };
  ...
  function setPhotoView(i, view) {
    setPhotos((prev) => prev.map((p, idx) => (idx === i ? { ...p, view } : p)));
  }
  ...
  async function runNarrowing(currentAnswers, currentNotes = userNotes) {
    ...
    // 対話で利用者が追加した自由記述(写真に写らない観察・AIの質問への回答)
    const notesText = (currentNotes && currentNotes.length > 0)
      ? currentNotes.map((n, i) => `(${i + 1}) ${n}`).join("\n")
      : "(追加説明なし)";

    const instruction = `
あなたは木材腐朽菌の同定を補助する助手です。判定者ではありません。
このシステムが扱うのは「緑化樹木の主要木材腐朽菌」です。
各写真の直前に[写真N：視点]のラベルがあります。特に「裏面(子実層托)」はヒダ/管孔/針/平坦の別を、「断面」は組織(肉)の色を判断する最重要の手掛かりとして重視してください。ラベルが「視点未指定」の写真は視点を推測して読み取ってください。
...
- 確認済みの決め手(人が確認した事実):
${answeredText}
- 利用者からの追加説明(対話。写真に写らない観察やAIの質問への回答。KOH反応・断面の色・剛毛体・匂い・触感・宿主樹種など):
${notesText}
...
6.5 「利用者からの追加説明」は、写真の印象より優先度の高い事実として扱い、候補と確信度を更新する。追加説明で決め手が確定したら、その種を上げる/外す。
7. 写真と観察情報だけでは足りない場合は、候補を分けるために現地で確認できる決め手の質問を1〜3つ出す(選択肢付き)。既に追加説明で分かったことは重ねて聞かない。
...
JSONのみ。前置き・後置き・コードフェンス(\`\`\`)は一切付けない。
{
 ...
 "kagi":{"pair":["",""],"chigai":"","kakunin":""},
 "hantei":"保留 または 確定候補",
 "comment":"次にすべきこと(打診・KOH・断面確認・採取など)を一言"
}
kagi(見分けの鍵)について:判定が「保留」、または上位2候補の確信度差が小さい(目安0.2以内)ときは必ず埋める。pairは拮抗する2種の和名、chigaiはその2種を分ける決定的な違い(1文)、kakuninは現地・ルーペ・KOH・断面など今すぐ確認できる方法(1文)。候補の kettede に基づき、顕微鏡が要る項目より現地で確認できる項目を優先する。拮抗していなければ pair は空配列[]、chigai/kakuninは空文字でよい。`.trim();

    // 各画像の直前に「視点(部位)ラベル」を差し込み、Geminiが何の写真か分かるようにする
    const parts = [];
    photos.forEach((p, i) => {
      const v = p.view && p.view !== "未指定" ? p.view : "視点未指定";
      parts.push({ text: `[写真${i + 1}：${v}]` });
    ...
    try {
      ...
      try {
        ...
        const aiText = [
          `判定:${parsed.hantei || ""}${parsed.comment ? "｜" + parsed.comment : ""}`,
          top ? `暫定1位:${top.wamei}(${Math.round((top.conf || 0) * 100)}%)` : "",
          (parsed.questions || []).length ? "確認したいこと:\n" + parsed.questions.map(q => `・${q.q}`).join("\n") : "",
        ].filter(Boolean).join("\n");
        setDialogue(prev => [...prev, { role: "ai", text: aiText }]);
      } catch {
  ...
  function answerQuestion(kettede, value) {
    ...
    setAnswers(next);
    setDialogue(prev => [...prev, { role: "user", text: `${kettede}：${value}` }]);
    runNarrowing(next, userNotes);
  }

  // 対話: 自由入力を送ってAIに再検討させる
  function sendChatNote() {
    const note = chatInput.trim();
    if (!note || loading) return;
    const nextNotes = [...userNotes, note];
    setUserNotes(nextNotes);
    setDialogue(prev => [...prev, { role: "user", text: note }]);
    setChatInput("");
    runNarrowing(answers, nextNotes);
  }

  // 「候補を絞る」: 対話ログ・追加説明をリセットして初回同定
  function startNarrowing() {
    setDialogue([]);
    setUserNotes([]);
    runNarrowing(answers, []);
  }
  ...
  function makeRecord({ wamei, gakumei, kenshou, confidence, ai_riyu, source, manual_reason }) {
    ...
    return {
      ...
      group:            groupKeyword,        // 共有キーワード(同じ値の人だけで共有)
      created_at:       now,
      updated_at:       now,
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "20px 16px 64px" }}>
        ...
        {activeTab === "identify" && (
          <>
            <section style={cardStyle()}>
              ...
              {/* 撮っておきたい視点チェックリスト(撮り忘れ防止) */}
              <div style={{ marginTop: 10, padding: "8px 10px", borderRadius: 6, background: "#F3F1E8", border: `1px solid ${C.line}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.rust, marginBottom: 4 }}>撮っておきたい視点（多いほど精度が上がります）</div>
                {PHOTO_VIEWS.map((v) => {
                  const have = photos.some((p) => p.view === v.key);
                  return (
                    <div key={v.key} style={{ fontSize: 12, color: have ? C.sage : C.sub, lineHeight: 1.7 }}>
                      {have ? "✓" : "○"} <b>{v.key}</b> <span style={{ color: C.sub }}>… {v.why}</span>
                    </div>
                  );
                })}
              </div>
              ...
              {photos.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 10 }}>
                  {photos.map((p, i) => (
                    <div key={i} style={{ position: "relative", width: 92 }}>
                      <img src={p.url} alt="" style={{ width: 92, height: 76, objectFit: "cover", borderRadius: 6, border: `1px solid ${C.line}` }} />
                      ...
                      <select value={p.view || "未指定"} onChange={(e) => setPhotoView(i, e.target.value)}
                              style={{ width: "100%", marginTop: 4, fontSize: 11, padding: "2px", borderRadius: 4, border: `1px solid ${C.line}` }}>
                        {PHOTO_VIEW_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
                      </select>
                    </div>
                  ))}
                </div>
            ...
            {result && (
              <section style={{ ...cardStyle(), marginTop: 16 }}>
                ...
                {/* 見分けポイントカード: 上位2種が拮抗するときの決定的な違いと確認方法 */}
                {result.kagi && result.kagi.chigai && (
                  <div style={{ marginBottom: 12, padding: 12, borderRadius: 8, background: "#FAF1D8", border: `1px solid ${C.amber}` }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.amber, marginBottom: 6 }}>
                      🔑 見分けポイント{Array.isArray(result.kagi.pair) && result.kagi.pair.filter(Boolean).length === 2 ? `：${result.kagi.pair[0]} ↔ ${result.kagi.pair[1]}` : ""}
                    </div>
                    <div style={{ fontSize: 13, lineHeight: 1.7, color: C.ink }}><b>決定的な違い：</b>{result.kagi.chigai}</div>
                    {result.kagi.kakunin && (
                      <>
                        <div style={{ fontSize: 13, lineHeight: 1.7, color: C.ink, marginTop: 4 }}><b>今すべき確認：</b>{result.kagi.kakunin}</div>
                        <button onClick={() => { setShowChat(true); setChatInput(`${result.kagi.kakunin}：`); }}
                                style={{ marginTop: 8, fontSize: 12, padding: "4px 10px", borderRadius: 6, border: `1px solid ${C.amber}`, background: "#fff", color: C.amber, cursor: "pointer" }}>
                          この確認結果を対話で伝える →
                        </button>
                      </>
                    )}
            ...
            {result && (
              <section style={{ ...cardStyle(), marginTop: 16 }}>
                <button onClick={() => setShowChat((v) => !v)}
                        style={{ background: "none", border: "none", color: C.rust, fontSize: 13, fontWeight: 700, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 6 }}>
                  {showChat ? "▼" : "▶"} 3. AIと対話して絞り込む
                </button>
                {showChat && (
                <>
                <p style={{ fontSize: 12, color: C.sub, marginTop: 10 }}>
                  写真に写らない特徴（KOH反応・断面の色・剛毛体・匂い・触感・宿主樹種など、現地でしか分からないこと）を伝えると、候補を絞り直します。AIの質問にボタンで答えても、下の欄に自由に書いてもOKです。
                </p>

                {/* 対話ログ */}
                {dialogue.length > 0 && (
                  <div style={{ border: `1px solid ${C.line}`, borderRadius: 8, padding: 10, marginBottom: 12, maxHeight: 320, overflowY: "auto", background: "#FAF9F4" }}>
                    {dialogue.map((m, i) => (
                      <div key={i} style={{ textAlign: m.role === "user" ? "right" : "left", margin: "6px 0" }}>
                        <span style={{ display: "inline-block", maxWidth: "88%", whiteSpace: "pre-wrap", textAlign: "left", padding: "6px 10px", borderRadius: 10, fontSize: 13, lineHeight: 1.6,
                          background: m.role === "user" ? C.sage : "#F3F1E8", color: m.role === "user" ? "#fff" : C.ink, border: m.role === "user" ? "none" : `1px solid ${C.line}` }}>
                          <b style={{ fontSize: 10, opacity: 0.7, display: "block" }}>{m.role === "user" ? "あなた" : "AI"}</b>
                          {m.text}
                        </span>
                      </div>
                    ))}
                    {loading && <div style={{ fontSize: 12, color: C.sub, marginTop: 4 }}>AIが再検討中…</div>}
                  </div>
                )}
                      ...
                      <div key={i} style={{ marginBottom: 12 }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 6 }}>
                          {(q.sentakushi || []).map((opt, j) => {
                            ...
                            return (
                              <button key={j} onClick={() => answerQuestion(q.kettede, opt)} disabled={loading}
                                      style={{ padding: "6px 12px", borderRadius: 16, fontSize: 13, cursor: loading ? "default" : "pointer", border: `1px solid ${selected ? C.sage : C.line}`, background: selected ? C.sage : "#fff", color: selected ? "#fff" : C.ink }}>
                                {opt}
                              </button>
                            );
                          })}
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  <input value={chatInput} onChange={(e) => setChatInput(e.target.value)}
                         onKeyDown={(e) => { if (e.key === "Enter" && !e.nativeEvent.isComposing) { e.preventDefault(); sendChatNote(); } }}
                         placeholder="例:断面はチョコレート色。KOHで黒変。宿主はスダジイ。" style={{ ...inputStyle(), flex: 1 }} disabled={loading} />
                  <button onClick={sendChatNote} disabled={loading || !chatInput.trim()} style={{ ...primaryBtn(loading || !chatInput.trim()), width: "auto", marginTop: 0, padding: "0 18px" }}>送信</button>
                </div>
                </>
                )}
              </section>
        ...
        {activeTab === "records" && (
          <section style={cardStyle()}>
            ...
            {/* 共有キーワード(グループ合言葉): 同じ値の人だけで記録を共有する */}
            <div style={{ marginBottom: 12, padding: "8px 12px", background: "#F3F1E8", borderRadius: 6, fontSize: 13, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <span><b>共有キーワード:</b>{" "}
                {groupKeyword
                  ? <>{groupKeyword}
                      <button onClick={() => {
                        const v = prompt("共有キーワードを変更(同じ値の人だけで記録を共有します):", groupKeyword);
                        if (v !== null) { const g = v.trim(); setGroupKeyword(g); localStorage.setItem("fungi_group", g); doSync(); }
                      }} style={{ marginLeft: 6, fontSize: 11, padding: "1px 6px", cursor: "pointer" }}>変更</button>
                    </>
                  : <span style={{ color: C.rust }}>未設定(共有なし)
                      <button onClick={() => {
                        const v = prompt("共有キーワードを入力(同じ値を入れた人だけで記録を共有します。空欄なら共有しません):");
                        if (v !== null) { const g = v.trim(); setGroupKeyword(g); localStorage.setItem("fungi_group", g); doSync(); }
                      }} style={{ marginLeft: 6, fontSize: 11, padding: "1px 6px", cursor: "pointer" }}>設定する</button>
                    </span>
                }
              </span>
              <span style={{ fontSize: 11, color: C.sub }}>同じキーワードの人とだけ記録が同期されます</span>
            </div>
                  ...
                  return (
                    <div key={rec.record_id} style={{ borderBottom: `1px solid ${C.line}`, padding: "10px 0" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        ...
                        {/* 削除ボタン: 登録者名を設定済みで、かつ自分の登録のときだけ表示(未設定時は非表示) */}
